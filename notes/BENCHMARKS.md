...
|---|---|---|---|---|---|---|---|---|
| LIS basin alert (this work) | chl-a > 10 ug/L, western LIS | 21 d | not re-run | not re-run | 9 of 41 | 2.14 | 41 basin-days, 9 events (2023-2025) | Rebuilt (S1) label, 2026-09-23 (was lift 1.37 on 12 events on the original sensor label). Initiation forecast; threshold t=0.65 pre-registered on 2020-2022 val. **Advantage over always-alert has a CI spanning zero** (+0.43 [-1.00, +1.77]); see reference rows |
| LIS station-day (this work) | same | 21 d | 0.744 | 0.883 | 0.045 | 2.59 | 954 station-days, 43 events (2023-2025) | S1 label, t*=0.35 (was POD 0.875, FAR 0.875, lift 2.7 on the original sensor label; t* is now an open decision). Beats always-alert clearly: paired lift diff +1.59 [+1.07, +2.22]. ver_far and empty-window fraction: sensor label; not re-run |
| — always-alert, basin (reference) | same | — | 1.00 | not re-run | 9 of 41 | 1.00 | same 41 basin-days | Requires no data; the floor |
| — persistence, basin (reference) | same | — | not re-run | not re-run | 9 of 41 | 2.10 | same 41 basin-days | "Was the last reading above 10?" — a tie with the basin model's 2.14 (was 1.81 vs 1.37 on the sensor label) |
...
the LIS basin-day result whose CI spans zero.
...
4. Small n on both sides: our basin test has 9 events (S1 label); the GoM
