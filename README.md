...
- Nine pipeline defects were found by systematic self-audit and fixed: (1) a positional row-shift label, (2) an uncensored 28-day label, (3) a shared label builder that never emitted NaN — which meant the operating point itself had been selected against fabricated negatives, (4) full-record climatology leakage in 4 of the locked 35 features, (5) a threshold swept on the test set, (6) a per-station strategy chosen on test F1, (7) a row-shift label plus wrong input grain in the script that freezes the operating point, (8) per-station thresholds fit on 0–5 validation positives, with 39 of 43 rows in `station_thresholds.csv` stale from a pre-fix run, and (9) `environment.yml` missing 9 packages the code imports. `src/archive/README.md` documents which scripts carry which; `tests/` guards labels, dependencies and station thresholds against regression.
- Per-station threshold tuning was tested and rejected: only 1 of 42 stations has enough validation positives to support it, and pooled it is strictly worse than a single global threshold (precision 0.176 vs 0.185 at equal recall 0.667). Together with the station-gate rejection, the ceiling is temporal, not spatial.
